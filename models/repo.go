...
func ComposeHTTPSCloneURL(owner, repo string, user string) string {
	if user != nil {
		return fmt.Sprintf("%s%s@%s/%s.git", setting.AppURL, user, owner, repo)
	} else {
...
func (repo *Repository) cloneLink(isWiki bool) *CloneLink {
	...
	cl.HTTPS = ComposeHTTPSCloneURL(repo.Owner.Name, repoName, nil)
	return cl
}
